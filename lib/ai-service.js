 ...
 * @param {Array} conversationContext - Array of previous conversation messages
 * @returns {Promise<{mermaidCode: string, error: string|null}>} - The generated Mermaid code or error
 */
export async function generateMermaidFromText(text, diagramType = "auto", onChunk = null, conversationContext = []) {
  if (!text) {
  ...
  if (!onChunk) {
    return generateMermaidTraditional(cleanedText, diagramType, aiConfig, accessPassword, selectedModel, conversationContext);
  }
  ...
  try {
    const response = await fetch("/api/generate-mermaid", {
      ...
      body: JSON.stringify({
        ...
        conversationContext // 传递对话上下文到后端
      }),
    });
 ...
 * @private
 */
async function generateMermaidTraditional(cleanedText, diagramType, aiConfig, accessPassword, selectedModel, conversationContext = []) {
  try {
    const response = await fetch("/api/generate-mermaid", {
      method: "POST",
      headers: {
